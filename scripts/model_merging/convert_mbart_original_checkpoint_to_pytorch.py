...
def convert_fairseq_mbart_checkpoint_from_disk(
    ...
    vocab_size = AutoTokenizer.from_pretrained(hf_config_path).vocab_size

    backup = state_dict["encoder.embed_tokens.weight"]
    state_dict["encoder.embed_tokens.weight"] = torch.zeros((vocab_size, backup.size(1)))
    state_dict["encoder.embed_tokens.weight"][:-1, :] = backup

    backup = state_dict["decoder.embed_tokens.weight"]
    state_dict["decoder.embed_tokens.weight"] = torch.zeros((vocab_size, backup.size(1)))
    state_dict["decoder.embed_tokens.weight"][:-1, :] = backup

    mbart_config = MBartConfig.from_pretrained(hf_config_path, vocab_size=vocab_size)
    if mbart_50 and finetuned:
        mbart_config.activation_function = "gelu"
